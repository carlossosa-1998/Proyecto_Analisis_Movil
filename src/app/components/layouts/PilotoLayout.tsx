...
export const PilotoLayout: React.FC<LayoutProps> = ({ children, currentView, onNavigate }) => {
    ...
    const [notificaciones, setNotificaciones] = useState([
        {
            ...
            detalle: "Has sido asignado a la misión #MIS-801 en Sector Norte - Lote A1.",
            tiempo: "Hace 10 min",
        ...
        },
        {
            ...
            detalle: "Tu plan de piloto VIP se renueva automáticamente el 01 de agosto.",
            tiempo: "Hace 2 horas",
    ...
    const bottomNavLeft: MenuItem[] = [
        { id: 'piloto-misiones', label: 'Misiones', icon: FileText },
        { id: 'piloto-dron', label: 'Drones', icon: Plane },
    ];

    const bottomNavRight: MenuItem[] = [
        { id: 'piloto-maps', label: 'Editor', icon: Pencil },
        { id: 'piloto-history', label: 'Historial', icon: History },
    ];
    ...
    return (
        <div className="flex flex-col h-full w-full bg-[#F8FAFC] font-['Roboto'] overflow-hidden relative">
            ...
            <nav className="absolute bottom-1 left-0 right-0 h-16 bg-white border-t border-slate-200 flex items-center justify-around px-2 z-20 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
                {/* Lado Izquierdo (Misiones y Drones) */}
                ...
                {/* Centro: Dashboard Piloto (Principal) */}
                <button
                    onClick={() => onNavigate('piloto-dashboard')}
                    className="flex flex-col items-center justify-center flex-1 h-full gap-0.5 group"
                >
                    <div
                        className={`w-9 h-9 rounded-[4px] flex items-center justify-center transition-all ${
                            currentView === 'piloto-dashboard'
                    ...
                    <span
                        className={`text-[9px] font-bold ${
                            currentView === 'piloto-dashboard' ? 'text-[#0E5E6F]' : 'text-slate-500'
                        }`}
                    >
                        Dashboard
                    </span>
                ...
                {/* Lado Derecho (Editor e Historial) */}
